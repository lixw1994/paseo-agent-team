...
export const roleSchema = z.enum(["researcher", "writer", "worker"]);
const storedRoleSchema = z.enum(["researcher", "writer", "worker", "reviewer", "implementer"]);
...
export const memberSchema = z.object({
  requestId: z.string().min(1), role: storedRoleSchema, task: z.string(),
...
export const startSchema = scopeSchema.extend({
  requestId: z.string().uuid(), role: roleSchema, task: z.string().trim().min(1).max(16000),
